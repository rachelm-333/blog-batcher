...
G3. STRUCTURAL HTML (MANDATORY — every article): You MUST include at least ONE <ul> or <ol> list. Present any comparison, pricing/specs, or at-a-glance summary as a BULLETED LIST where each item begins with a bold label, e.g. <li><strong>Option A</strong> — key detail; another detail</li>. Do NOT use HTML <table> / <tr> / <td> elements anywhere — deliver all tabular or comparison data as this bold-label bulleted list instead. AI answer engines extract these labelled lists just as well as tables, and they render reliably on every platform.
