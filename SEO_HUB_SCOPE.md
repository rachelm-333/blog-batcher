...
- **Cluster format diversity (STRICT anti-cannibalization):** under each pillar, every cluster MUST use a DIFFERENT content format — never the same format twice under one pillar. Each cluster = unique format + distinct keyword + distinct search intent (no two clusters share a SERP). Formats: **A How-To** (step-by-step), **B Comparison** (X vs Y / alternatives), **C Listicle/Examples** (roundup/tools/stats), **D Troubleshooting** (fix a problem), **E Cost/ROI** (pricing/value). The hub generator assigns a unique format to each cluster and reflects it in the keyword + title + article type.
